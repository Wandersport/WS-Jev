...
- Polymarket order book consensus capture
- Binance USD-M Perpetual microstructure capture
- Point-in-time immutable snapshot construction
...
class BTC5mShadowLab:
    ...
    def probe_connectivity(self) -> dict[str, Any]:
        ...
        # 2. Probe CLOB order book for active tokens
        try:
            ...
            results["details"]["market_q"] = state.market_q
        ...
        try:
            binance_feats = self.binance_feed.fetch_rest_snapshot(
                ...
                round_start_price=round_info.price_to_beat,
            )
    ...
    def capture_horizon_snapshot(
        ...
        # Extract reference features (use priceToBeat from event metadata or opening anchor)
        ...
        # Extract Binance Perpetual microstructure
    ...
    def monitor_round(
        ...
        for h in horizons:
            # Calculate target scheduled time: round_end_epoch - h
    ...
    def score_resolved_round(
        ...
        forecasts = self.db.get_btc5m_forecasts(round_slug=round_slug)

        # Group forecasts by snapshot_id and condition
    ...
    def compute_evaluation_summary(
        ...
        """Compute aggregated calibration metrics and clustered bootstrap confidence intervals."""
        scores = self.db.get_btc5m_resolution_scores()
        if not scores:
            return {"status": "NO_RESOLVED_DATA", "total_scores": 0}

        # Group by round_slug for clustered bootstrap
            ...
            for cond in conditions:
                ...
                for s in sample_scores:
                    ...
                    if mb is not None and cb is not None:
                        m_briers.append(float(mb))
                        c_briers.append(float(cb))

                if m_briers and c_briers:
                    mean_diff = sum(c_briers) / len(c_briers) - sum(m_briers) / len(m_briers)
                    boot_diffs[cond].append(mean_diff)

        # Baseline sample calculations
        for cond in conditions:
            _p_key, brier_key, log_loss_key = cond_keys[cond]
            c_briers = [
                float(s[brier_key]) for s in scores if s.get(brier_key) is not None
            ]
            c_losses = [
                float(s[log_loss_key]) for s in scores if s.get(log_loss_key) is not None
            ]
            m_briers = [
                float(s["market_brier"])
                for s in scores
                if s.get("market_brier") is not None and s.get(brier_key) is not None
            ]
            m_losses = [
                float(s["market_log_loss"])
                for s in scores
                if s.get("market_log_loss") is not None and s.get(log_loss_key) is not None
            ]
